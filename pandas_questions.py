...
def merge_regions_and_departments(regions, departments):
    ...
    """
    regions_split = regions[["code", "name"]]
    regions_split.rename(
        columns={"code": "code_reg", "name": "name_reg"}, inplace=True
    )
    departments_split = departments[["region_code", "code", "name"]]
    departments_split.rename(
        columns={
    ...
    merged = pd.merge(
        regions_split, departments_split, on="code_reg", how="left"
    )
    return merged
...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    df_referendum = referendum
    df_referendum["code_dep"] = df_referendum["Department code"].apply(
        lambda s: "0" + s if len(s) == 1 else s
    )
    merged = pd.merge(df_referendum, regions_and_departments, on="code_dep")
    return merged
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    """
    file = open("data/regions.geojson", mode="rb")
