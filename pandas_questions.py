...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    return referendum_and_areas.groupby('name_reg').sum().reset_index()
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    geographic_data = geographic_data.rename(columns={'nom': 'name_reg'})
    expressed_ballots = referendum_result_by_regions['Choice A'] + \
        referendum_result_by_regions['Choice B']
    referendum_result_by_regions['ratio'] = \
        referendum_result_by_regions['Choice A'] \
        / expressed_ballots
    gdf_referendum = gpd.GeoDataFrame(
        referendum_result_by_regions.merge(geographic_data, on='name_reg')
    )
