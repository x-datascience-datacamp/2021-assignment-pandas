...
def merge_regions_and_departments(regions, departments):
    ...
    merged = pd.merge(left, right, left_on='code', right_on='region_code',
                      suffixes=('_reg', '_dep'))
...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    left.loc[:, 'Department code'] = left.loc[:, 'Department code']\
        .apply(lambda x: '0' + x if len(x) == 1 else x)
    to_drop_right = ['01', '02', '03', '04', '05', '06', 'COM']
    right = regions_and_departments[
        ~regions_and_departments['code_reg'].isin(to_drop_right)
    ]
    merged = pd.merge(
        left, right, left_on='Department code', right_on='code_dep')
    return merged
