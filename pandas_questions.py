...
def merge_regions_and_departments(regions, departments):
    ...
    """
    regions['code'] = regions['code'].apply(lambda x: x.zfill(3))
    departments['region_code'] = \
        departments['region_code'].apply(lambda x: x.zfill(3))
    df = pd.merge(
...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    referendum['Department code'] = \
        referendum['Department code'].apply(lambda x: x.zfill(3))
    regions_and_departments['code_dep'] = \
        regions_and_departments['code_dep'].apply(lambda x: x.zfill(3))
    df = pd.merge(
...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    Registered = pd.to_numeric(referendum_and_areas['Registered'])
    referendum_and_areas['Registered'] = Registered
